...
def ges_krankenv_beitr_selbst(krankenv_pflichtig_eink_selbst, params):
    ...
    return pd.Series(
        index=krankenv_pflichtig_eink_selbst.index,
        data=out,
        name="ges_krankenv_beitr_selbst",
    )


def pflegev_beitr_selbst(hat_kinder, alter, krankenv_pflichtig_eink_selbst, params):
    """
...
def krankenv_pflichtig_eink_selbst(eink_selbstst_m, bezugsgröße, params):
    """
    ...
    return pd.Series(
        index=eink_selbstst_m.index, data=out, name="krankenv_pflichtig_eink_selbst"
    )


def krankv_pflichtig_rente(ges_rente_m, krankenv_beitr_bemess_grenze, params):
    """
    Choose the amount pension which is subject to health insurance contribution.
    ...
    """
    out = elementwise_min(ges_rente_m, krankenv_beitr_bemess_grenze)
...
def krankenv_beitr_bemess_grenze(wohnort_ost, params):
    ...
    return pd.Series(
        index=wohnort_ost.index, data=out, name="krankenv_beitr_bemess_grenze"
    )
...
def ges_krankenv_beitr_rente(krankv_pflichtig_rente, params):
    """
    Calculating the contribution to health insurance for pension income.
    ...
    return pd.Series(
        index=krankv_pflichtig_rente.index, data=out, name="ges_krankenv_beitr_rente"
    )
